...
STATION_SHA="34dc3a40a4d9b5f1dc770303ecbc60e3b7263eca2cb3577ec140358a59037370"
...
pin_for() {
    case "$1" in
        railcall_cli.py)                          echo 4a6ce9383e752995e3ea0c41ea5adb742140de985b63922f23c39b06ad488bcd ;;
...
STATION_URL="https://github.com/patl4588/railcall-core/releases/download/station-v0.30/railcall_station.tar.gz"
...
if [ -z "${RAILCALL_NO_TELEMETRY:-}" ]; then
    ...
    if [ -n "${MID:-}" ]; then
        ...
        curl -fsS --max-time 3 -o /dev/null \
            ...
            -d "{\"machine_id\":\"$MID\",\"version\":\"0.31.0\",\"station_sha\":\"$STATION_SHA\",\"os\":\"$OS\",\"arch\":\"$ARCH\"}" \
