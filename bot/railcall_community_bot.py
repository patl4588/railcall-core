...
SYSTEM_PROMPT = (
    ...
    "• Billing is blind-metered: a flat $0.01 per governed run. The client sends only a hashed key + a "
    "one-time nonce to check balance — never the raw key, files, or data. It's a transaction register, "
    "not a data sink.\n"
    "• Free tier = 100 runs, no card. Runs are prepaid; balance never expires; no per-seat fees.\n"
